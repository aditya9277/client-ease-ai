...
export const LiveKnowledgeBaseCard = () => {
  ...
  useEffect(() => {
    ...
    const fetchSuggestions = async () => {
      try {
        const { data } = await axios.get(`http://localhost:5000/knowledge-base/search?query=${searchQuery}`);
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const { data } = await axios.get(`http://localhost:5000/knowledge-base/query?query=${searchQuery}`);
